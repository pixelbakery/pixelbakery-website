...
function Obfuscate(props) {
  ...
  const generateLink = ({ email, headers, tel, sms, facetime, href, children }) => {
    let link
  ...
  const obfuscatedStyle = {
    ...
    direction:
      humanInteraction === true || obfuscate === false || obfuscateChildren === false
        ? 'ltr'
        : 'rtl',
  }
      ...
      ? {
          href:
            humanInteraction === true || obfuscate === false
              ? generateLink({ email, headers, tel, sms, facetime, href, children })
              : linkText || 'obfuscated',
          onClick: handleClick,
        }
      : {}
