...
function Navbar() {
  ...
  useLayoutEffect(() => {
    ...
    navTL.to("#logo", {y:0, opacity:1, duration: 1, delay: 1, ease: "back.out"})
    .to("#logo-line-1", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    .to("#logo-line-2", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    .to("#nav-link-1", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    .to("#nav-link-2", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    .to("#nav-link-3", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    .to("#nav-ham", {opacity:1, y:0,duration:.33, ease: "back.out,"}, "<33%")
    document.onload = function(){
      ...
      tl.addLabel("start")
      .to("#esign", {opacity:0, duration: .4}, "<25%")
      .to("#tudio", {opacity:0, duration: .4}, "<25%")
      .to("#ixel", {opacity:0, duration: .4}, "<25%")
      .to("#akery", {opacity:0, duration: .4}, "<25%")
      .to("#b", {x: -92, ease: "sine.inOut", duration: .25}, "<25%")
      .from("#design-studio", {scale:.75, x:-26, y:-3, duration: .25}, "<25%")
      .to("#d", {x: 3, ease: "sine.inOut", duration: .25}, "<")
      .to("#s", {x: -140, scaleY: 1.25, y:-4, ease: "sine.inOut", duration: .25}, "<33%")
      .to("#logo-bg", {height:124, width: 90, ease: "sine.inOut", duration: .25}, "<")
      .from("#logo-wrapper", {y:-9, ease: "sine.inOut", duration: .25}, "<")
      .from("#logo", {scale: .8, ease: "sine.inOut", duration: .25}, "<")
      .to("#nav-link-1", {x:300, scale:0, opacity:0, ease: "back.in", duration: .5}, "<")
      .to("#nav-link-2", {x:300, scale:0, opacity:0, ease: "sine.inOut", duration: .5}, "<25%")
      .to("#nav-link-3", {y:-20, opacity:0, ease: "back.in", duration: .33}, "<33%")
      .addLabel("end");
      ...
      <Link to="/" className="block relative  h-full w-full z-50 px-4 py-4">
          <div  id="logo-wrapper" className="relative z-20 top-0 bg-none text-peach text-center font-extrabold rounded-md  text-5xl leading-none inline-block">
            <p id="logo-line-1" className="opacity-0 tracking-normal text-left">
              ...
              <span id="ixel" className="inline-block">ixel </span> 
              <span id="b" className="inline-block ml-3">b</span>
              <span id="akery" className="inline-block">akery</span>
            </p>
            <p id="logo-line-2" className="tracking-wide -mt-2 -ml-1 text-left" id="design-studio">
              <span id="d" className="inline-block">d</span>
              <span id="esign" className="inline-block">esign </span> 
              <span id="s" className="inline-block ml-3">s</span>
              <span id="tudio" className="inline-block">tudio</span>
